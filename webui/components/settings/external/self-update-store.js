...
const model = {
  ...
  get quickStatusMessage() {
    ...
    if (!this.isSupported) {
      return "Self-update is currently available only in dockerized Synapse deployments that boot through /exe/run_A0.sh.";
    }
    ...
    if (this.quickUpdateComparison > 0) {
      ...
      return `This instance is significantly behind the latest release currently available on main. Restart Synapse to move to ${this.mainBranchLatestVersion}.`;
    }
    if (this.quickUpdateComparison === 0) {
  ...
  updateProgressOverlay() {
    ...
    if (detail) {
      detail.textContent =
        this.restartDetailText ||
        "Synapse is restarting, applying the requested release, and will reload this page when the health check responds again.";
    }
  },
  ...
  async scheduleUpdateRequest(payload, notificationMessage) {
    ...
    this.setRestartState(
      "Preparing update",
      "Saving the request and asking Synapse to restart."
    );
    this.ensureProgressOverlay();
    try {
  ...
  async scheduleUpdate() {
    ...
    await this.scheduleUpdateRequest(
      ...
      "Synapse is restarting to apply the requested branch and version target.",
    );
  },
  ...
  async scheduleQuickUpdate() {
    ...
    await this.scheduleUpdateRequest(
      ...
      "Synapse is restarting to apply the latest version from main.",
    );
  },
  ...
  async restartAndReload() {
    ...
    this.setRestartState(
      ...
      "The request was saved. Synapse is about to restart and apply the requested branch and version target."
    );
    this.ensureProgressOverlay();
    ...
    try {
      ...
      if (restartResponse && !restartResponse.ok) {
        if (restartResponse.status >= 500) {
          console.warn(
            `Restart request returned HTTP ${restartResponse.status} while Synapse was shutting down. Continuing to wait for the new runtime.`
          );
          this.setRestartState(
            "Restarting backend",
            "Synapse is shutting down and applying the update. Waiting for the new runtime to come back healthy."
          );
        } else {
          throw new Error(
        ...
        this.setRestartState(
          "Restarting backend",
          "Synapse accepted the restart request. Waiting for the updater to take over."
        );
      }
    } catch (error) {
      ...
      console.warn(
        "Restart request connection closed while Synapse was restarting:",
        error
      );
    }
    ...
    this.setRestartState(
      "Update in progress",
      "Synapse is restarting and the updater is running. This page will reload automatically when /api/health starts responding again."
    );
    ...
    while (Date.now() < deadline) {
      try {
        ...
        if (response.ok) {
          this.setRestartState(
            "Restarting backend",
            "Waiting for Synapse to disconnect before reloading the page."
          );
          ...
          this.setRestartState(
            "Update in progress",
            "Synapse is restarting and the updater is running. This page will reload automatically when the health check becomes healthy again."
          );
        ...
        this.setRestartState(
          "Update in progress",
          "Synapse is temporarily unavailable while it restarts. Waiting for the new runtime to become healthy."
        );
    ...
    this.removeProgressOverlay();
    this.error =
      "Synapse did not come back within the expected window. It may still be rolling back. " +
