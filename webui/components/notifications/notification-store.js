...
const model = {
  ...
  formatTimestamp(timestamp) {
    ...
    if (diffMins < 0.15) return "Только что";
    else if (diffMins < 1) return "Меньше минуты назад";
    else if (diffMins < 60) return `${Math.round(diffMins)}м назад`;
    else if (diffHours < 24) return `${Math.round(diffHours)}ч назад`;
    else if (diffDays < 7) return `${Math.round(diffDays)}д назад`;
