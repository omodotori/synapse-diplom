...
Communicate with Synapse via Telegram. Supports polling and webhook modes with per-user chat sessions, group mentions, inline keyboards, and file exchange.

## What It Does

This plugin connects one or more Telegram bots to Synapse. Each bot runs independently with its own token, mode (polling or webhook), access control list, and project binding. Users message the bot on Telegram, and Synapse processes the conversation just like a WebUI chat — including tool use, sub-agents, and file attachments.
...
- **Project binding**
  - Map individual Telegram users to specific Synapse projects; supports a default-project fallback.
...
- **Description**: Communicate with Synapse via Telegram. Supports polling and webhook modes with per-user chat sessions.
