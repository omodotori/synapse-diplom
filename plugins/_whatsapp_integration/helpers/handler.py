...
async def _start_new_chat(config: dict, msg: dict) -> None:
    ...
    user_msg = _build_user_message(context.synapse, msg)
    system_ctx = context.synapse.read_prompt("fw.wa.system_context.md")
...
async def send_wa_reply(
    ...
    if mode == "self-chat":
        response_text = context.synapse.read_prompt(
