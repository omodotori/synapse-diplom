"""Context event streaming bridge for the synapse-connector plugin."""
...
def get_context_log_entries(
    ...
    try:
        ...
        PrintStyle.error(
            f"[synapse-connector] event_bridge error for context {context_id}: {exc}"
        )
        ...
        for event in events:
            if emit_fn is not None:
                try:
                    ...
                    PrintStyle.error(f"[synapse-connector] emit_fn error: {exc}")
            yield event
