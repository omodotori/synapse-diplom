...
This is different from `text_editor` which operates on the Synapse server's filesystem.
...
- Paths are evaluated on the **remote machine's filesystem**, not the Synapse server.
