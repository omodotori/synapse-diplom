...
async def _handle_control_email(
    ...
    if not target_context_id:
        if len(existing_chats) == 1:
            ...
            await _send_control_email_reply(
                handler_cfg,
                msg,
                "Multiple Synapse email chats match this sender. Reply inside the thread you want to control.",
                thread_id=thread_id,
            )
            return True
        else:
            await _send_control_email_reply(
                handler_cfg,
                msg,
                "No matching Synapse email chat was found. Reply inside an existing Synapse email thread to use /project, /config, or /send.",
                thread_id=thread_id,
            )
            return True
    ...
    if not context:
        await _send_control_email_reply(
            handler_cfg,
            msg,
            "The matching Synapse email chat is no longer available. Send a normal email to start a fresh thread.",
            thread_id=thread_id,
        )
        return True
...
def _apply_handler_model_preset(context: AgentContext, handler_cfg: dict) -> None:
    ...
    if not preset_name:
        return
    if not model_config.is_chat_override_allowed(context.synapse):
        PrintStyle.warning(
