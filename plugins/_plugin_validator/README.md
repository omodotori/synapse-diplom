...
Validate Synapse plugins against structural, manifest, convention, and security expectations.

## What It Does

This plugin generates a structured validation prompt for either a local plugin or an external source, runs the review in a temporary agent context, and returns a markdown report that checks whether a plugin follows Synapse plugin conventions.
...
- **Description**: Validate Synapse plugins against manifest, structure, code pattern, and security conventions.
