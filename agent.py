...
class AgentContext:
    ...
    def __init__(
        ...
        self.synapse = synapse or Agent(0, self.config, self)
    ...
    def reset(self):
        self.kill_process()
        self.log.reset()
        self.synapse = Agent(0, self.config, self)
    ...
    def nudge(self):
        self.kill_process()
        self.paused = False
        self.task = self.communicate(UserMessage(self.synapse.read_prompt("fw.msg_nudge.md")))
        return self.task

    @extension.extensible
    def get_agent(self):
        return self.streaming_agent or self.synapse
...
class Agent:
    ...
    def __init__(
        ...
        # agent context
        self.context = context or AgentContext(config=config, synapse=self)
