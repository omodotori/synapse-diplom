...
mcp_server: FastMCP = FastMCP(
    name="Synapse integrated MCP Server",
    instructions="""
    Connect to remote Synapse instance.
    Synapse is a general AI assistant controlling it's linux environment.
    Synapse can install software, manage files, execute commands, code, use internet, etc.
    Synapse's environment is isolated unless configured otherwise.
    """,
)
...
class ToolResponse(BaseModel):
    ...
    response: str = Field(
        description="The response from the remote Synapse Instance"
    )
...
class ToolError(BaseModel):
    ...
    error: str = Field(
        description="The error message from the remote Synapse Instance"
    )
    chat_id: str = Field(description="The id of the chat this message belongs to.")


SEND_MESSAGE_DESCRIPTION = """
Send a message to the remote Synapse Instance.
This tool is used to send a message to the remote Synapse Instance connected remotely via MCP.
"""
...
async def send_message(
    message: Annotated[
        str,
        Field(
            description="The message to send to the remote Synapse Instance",
            title="message",
        ),
    ],
    attachments: (
        Annotated[
            list[str],
            Field(
                description="Optional: A list of attachments (file paths or web urls) to send to the remote Synapse Instance with the message. Default: Empty list",
                title="attachments",
            ),
        ]
        | None
    ...
    Field(
        description="The response from the remote Synapse Instance", title="response"
    ),
]:
...
Finish a chat with the remote Synapse Instance.
This tool is used to finish a persistent chat (send_message with persistent_chat=True) with the remote Synapse Instance connected remotely via MCP.
If you want to continue the chat, use the send_message tool instead.
Always use this tool to finish persistent chat conversations with remote Synapse.
"""
