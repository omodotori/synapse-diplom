...
OFFICIAL_REPO_AUTHOR = "synapseai"
OFFICIAL_REPO_NAME = "synapse"
BRANCH_OPTIONS = [
...
UPDATE_FILE_PATH = Path("/exe/synapse-self-update.yaml")
STATUS_FILE_PATH = Path("/exe/synapse-self-update-status.yaml")
LOG_FILE_PATH = Path("/exe/synapse-self-update.log")
...
def _get_remote_branch_merged_tags(branch: str) -> set[str]:
    ...
    with tempfile.TemporaryDirectory(prefix="synapse-self-update-tags-") as temp_dir:
...
def _get_remote_branch_head_info(branch: str) -> dict[str, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="synapse-self-update-head-") as temp_dir:
        repository = Path(temp_dir)
        _run_git(repository, "init", "--bare")
        _run_git(
            repository,
