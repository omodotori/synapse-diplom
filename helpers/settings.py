...
def _adjust_to_version(settings: Settings, default: Settings):
    # starting with 0.9, the default prompt subfolder for agent no. 0 is synapse
    # switch to synapse if the old default is used from v0.8
    if "version" not in settings or settings["version"].startswith("v0.8"):
        if "agent_profile" not in settings or settings["agent_profile"] == "default":
            settings["agent_profile"] = "synapse"
...
def get_default_settings() -> Settings:
    ...
    return Settings(
        ...
        auth_password="",
        root_password="",
        agent_profile=get_default_value("agent_profile", "synapse"),
