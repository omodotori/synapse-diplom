...
def launch_ui_process(repo_dir: Path, logger: AttemptLogger) -> subprocess.Popen[bytes]:
    ...
    logger.log("Starting Synapse UI")
    ...
    *,
    ...
    try:
        ...
        if healthy:
            record_result(
                status="success",
                message=f"Updated Synapse to branch {branch}, {resolved_target['target_description']}.",
...
def trigger_update_command(args: list[str]) -> int:
    parser = argparse.ArgumentParser(
        ...
        description="Queue an Synapse self-update for the next startup attempt.",
    )
    parser.add_argument(
        "branch",
        nargs="?",
    ...
    print("Queued Synapse self-update for the next startup attempt.")
    ...
    print("Restart the container or Synapse process to apply it.")
    return 0
