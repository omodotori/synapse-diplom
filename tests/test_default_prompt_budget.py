...
def _iter_prompt_files():
    ...
    yield from (PROJECT_ROOT / "agents" / "synapse" / "prompts").rglob("*.md")
...
async def _build_system_text(profile: str = "synapse") -> str:
    ...
    try:
        system = await ctx.synapse.get_system_prompt(ctx.synapse.loop_data)
        return "\n\n".join(system)
    finally:
...
@pytest.mark.asyncio
async def test_default_synapse_prompt_budget_and_guardrails():
