# Development manual for Synapse
This guide will show you how to setup a local development environment for Synapse in a VS Code compatible IDE, including proper debugger.
...
> [!NOTE]
> - Synapse runs in a Docker container, this simplifies installation and ensures unified environment and behavior across systems.
...
2. Prepare the python environment to run Synapse in. (⚠️ This step assumes you have some Python runtime installed.) By clicking the python version in lower right corner (3.13.1 in my example), you should get a list of available environments. You can click the `+ Create Virtual Environment` button. You might be prompted to select the environment manager if you have multiple installed. I have venv and Conda, I will select Conda here. I'm also prompted for desired python version, I will select 3.12, that is known to work well.
...
## Step 4: Run Synapse in the IDE
Great work! Now you should be able to run Synapse from your IDE including real-time debugging.
...
After inserting my API key in settings, my Synapse instance works. I can send a simple message and get a response.
...
## Step 5: Run another instance of Synapse in Docker
...
Synapse runs code inside the container by default. If you are running the framework locally in your IDE but want tools (like code execution) to run in Docker, configure RFC in **Settings -> Development** and point it to a running Synapse container. This routes execution through SSH/RFC to the container while keeping the UI and agent loop on your host.


# Congratulations!

You have successfully set up a complete Synapse development environment! You now have:
...
- The ability to develop, debug, and test Synapse features seamlessly

You're now ready to contribute to Synapse, create custom extensions, or modify the framework to suit your needs. Happy coding!
