...
The install script is the fastest way to get Synapse running. It handles Docker, image pulling, and container setup automatically.
...
If you are upgrading from an older version of Synapse (v0.9.8 or earlier) to v1.1 or newer, the architecture has fundamentally changed. You cannot use the in-app Self Update. Instead, follow these steps to migrate your data:
...
Docker Desktop provides the runtime environment for Synapse, ensuring consistent behavior and security across platforms. The entire framework runs within a Docker container, providing isolation and easy deployment.
...
#### 2.1. Pull the Synapse Docker Image
...
Choose or create a folder on your computer where Synapse will save its data. 
...
> [!NOTE]
> After starting the container, you'll find all Synapse files in your chosen directory. You can access and edit these files directly on your machine, and the changes will be immediately reflected in the running container.
...
Synapse provides a comprehensive settings interface to customize various aspects of its functionality. Access the settings by clicking the "Settings" button with a gear icon in the sidebar.

### Agent Configuration

- **Agent Profile:** Select the agent profile (e.g., `synapse`, `hacker`, `researcher`). Profiles can override prompts, tools, and extensions.
...
> [!NOTE]
> Synapse uses a local embedding model by default (runs on CPU), but you can switch to OpenAI embeddings like `text-embedding-3-small` or `text-embedding-3-large` if preferred.
...
> [!NOTE]
> **OpenAI API vs Plus subscription:** A ChatGPT Plus subscription does not include API credits. You must provide a separate API key for OpenAI usage in Synapse.
...
> [!CAUTION]
> **GitHub Copilot Provider:** When using the GitHub Copilot provider, after selecting the model and entering your first prompt, the OAuth login procedure will begin. You'll find the authentication code and link in the output logs. Complete the authentication process by following the provided link and entering the code, then you may continue using Synapse.
...
The Settings page is the control center for selecting the Large Language Models (LLMs) that power Synapse. You can choose different LLMs for different roles:
...
> [!NOTE]
> If Synapse runs in Docker and Ollama runs on the host, ensure port **11434** is reachable from the container. If both services are in the same Docker network, you can use `http://<container_name>:11434` instead of `host.docker.internal`.
...
## Using Synapse on Your Mobile Device

Synapse can be accessed from mobile devices and other computers using the built-in **Tunnel feature**.

### Recommended: Using Tunnel (Remote Access)

The Tunnel feature allows secure access to your Synapse instance from anywhere:
...
5. Share this URL to access Synapse from any device
...
For developers or users who need to run Synapse directly on their system, see the [In-Depth Guide for Full Binaries Installation](dev-setup.md).

---

## Advanced: Automated Configuration via Environment Variables

Synapse settings can be automatically configured using environment variables with the `A0_SET_` prefix in your `.env` file. This enables automated deployments without manual configuration.
...
After following the instructions for your specific operating system, you should have Synapse successfully installed and running. You can now start exploring the framework's capabilities and experimenting with creating your own intelligent agents.
