# Synapse — Frontend Modals (stacked `openModal/closeModal`)

This document covers the stacked modal system used by Synapse’s frontend, implemented in:
...
Synapse uses a custom `<x-component path="...">` tag that is populated by the component loader.
...
Synapse can open modals from many places (sidebar, settings, message actions), and sometimes a modal opens another modal.
