...
Synapse uses a convention-over-configuration plugin model where runtime capabilities are discovered from the directory structure.
...
Plugins can include an optional `hooks.py` file at the plugin root. Synapse loads this module on demand and calls exported functions by name through `helpers.plugins.call_plugin_hook(...)`.

- `hooks.py` runs inside the **Synapse framework runtime and Python environment**, not the separate agent execution environment.
- Use it for framework-internal operations such as install-time setup, pre-update cleanup or preparation, plugin registration work, filesystem preparation, cache updates, or other tasks that need access to Synapse internals.
...
- If `hooks.py` installs Python packages with `sys.executable -m pip`, those packages are installed into the **same Python environment that runs Synapse itself**.
...
**Runtime manifest** (`plugin.yaml`, inside your plugin repo/directory — drives Synapse behavior):
```yaml
...
Both routes surface Plugin Index entries inside Synapse. The Plugin Hub supports search, filtering, sorting, and a detail view with README content and installation actions.
