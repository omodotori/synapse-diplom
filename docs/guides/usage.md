# Usage Guide
This guide explores usage and configuration scenarios for Synapse. You can consider this as a reference post-installation guide.

![Utility Message with Solutions](../res/usage/first-task.png)

## Basic Operations
Synapse provides several basic operations through its interface:

### Restart Framework
The Restart button allows you to quickly restart the Synapse container without using the terminal:
...
### Action Buttons
Located beneath the chat input box, Synapse provides a set of action buttons for enhanced control and visibility:
...
### File Browser: Manage files in the Synapse environment
...
### File Attachments
Synapse supports direct file attachments in the chat interface for seamless file operations:
...
## Tool Usage
Synapse's power comes from its ability to use [tools](../developer/architecture.md#tools). Here's how to leverage them effectively:

- **Understand Tools:** Synapse includes default tools like knowledge (powered by SearXNG), code execution, and communication. Understand the capabilities of these tools and how to invoke them.
...
Synapse instances can communicate with each other using the A2A protocol. This enables:
...
To enable A2A connectivity, go to **Settings → MCP/A2A → A0 A2A Server** and toggle the server on. You'll receive a connection URL that other Synapse instances can use to communicate with your agent.

See [A2A Setup](a2a-setup.md) for detailed configuration and use cases.

## Example of Tools Usage: Web Search and Code Execution
Let's say you want Synapse to perform some financial analysis tasks. Here's a possible prompt:
...
> [!NOTE]
> The first run of `code_execution_tool` may take a while as it downloads and builds the Synapse Docker image. Subsequent runs will be faster.

This example demonstrates how to combine multiple tools to achieve an analysis task. By mastering prompt engineering and tool usage, you can unlock the full potential of Synapse to solve complex problems.

## Multi-Agent Cooperation
One of Synapse's unique features is multi-agent cooperation.
...
* **Hierarchy:** Synapse uses a [hierarchical structure](../developer/architecture.md#agent-hierarchy-and-communication), with superior agents delegating tasks to subordinates.  This allows for structured problem-solving and efficient resource allocation.

![](../res/usage/multi-agent.png)

## Projects
Projects are isolated workspaces that provide dedicated context, instructions, memory, and secrets for specific tasks or clients. They are one of Synapse's most powerful organizational features, preventing context bleed and enabling focused, specialized agent behavior.
...
3. Synapse clones the repository and sets up the project structure
...
> [!NOTE]
> If the cloned repository already contains a `.a0proj/` configuration folder, Synapse merges the existing configuration with your specified preferences.
...
## Tasks & Scheduling
Tasks enable Synapse to run automated or scheduled work in isolated contexts. They're perfect for recurring workflows, batch processing, or time-delayed operations that don't require immediate attention.

### What Are Tasks?

Tasks are autonomous work units that Synapse executes in dedicated or shared chat contexts. Each task includes:
...
Synapse supports three types of tasks:
...
Synapse includes a secure tunneling feature that allows you to expose your local instance to the internet. This makes it possible to access your Synapse instance from anywhere or share it with others without complex network configuration.

### How Tunneling Works

Synapse uses the [Flaredantic](https://pypi.org/project/flaredantic/) library to create secure tunnels. These tunnels:
...
6. The tunnel URL remains active until you stop the tunnel or close Synapse

### Security Considerations

When sharing your Synapse instance via a tunnel:

- Anyone with the URL can access your Synapse instance
- No additional authentication is added beyond what your Synapse instance already has
- **Always set up authentication before creating a tunnel** (see below)
- The tunnel exposes only your Synapse instance, not your entire system

> [!IMPORTANT]
> When attempting to create a tunnel without authentication configured, Synapse will display a security warning.

### Adding Authentication for Tunnels

To secure your tunneled Synapse instance, configure authentication in Settings:

1. Open **Settings** in the Synapse UI
...
This will require users to enter these credentials when accessing your tunneled Synapse instance.
...
> [!TIP]
> Combine tunneling with authentication for secure remote access to your Synapse instance from any device, including mobile phones and tablets.

## Voice Interface
Synapse provides both Text-to-Speech (TTS) and Speech-to-Text (STT) capabilities for natural voice interaction:
...
### File Browser
Synapse provides a powerful file browser interface for managing your workspace:
...
> [!TIP]
> The File Browser integrates seamlessly with Synapse's capabilities. You can reference files directly in your conversations, and the agent can help you manage, modify, and organize your files.

## Memory Management
Synapse includes a sophisticated memory management system that stores and retrieves information from conversations, knowledge sources, and learning experiences. The Memory Dashboard provides a powerful interface to view, search, filter, edit, and delete memory entries stored in the vector database.
...
## Backup & Restore
Synapse provides a comprehensive backup and restore system to protect your data and configurations. This feature helps you safeguard your work and migrate Synapse setups between different systems.
...
#### What Gets Backed Up
By default, Synapse backs up your most important data:
...
* **Before System Updates**: Backup before updating Synapse or system components
...
* **External Storage**: Keep backup files in a safe location outside Synapse
...
#### System Migration
Moving Synapse to a new server or computer:
1. Create a complete backup on the original system
2. Install Synapse on the new system
...
#### Team Collaboration
Sharing Synapse configurations:
...
> [!TIP]
> The backup system is designed to work across different operating systems and Synapse installations. Your backups from a Windows system will work on Linux, and vice versa.
