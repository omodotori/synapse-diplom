...
This guide provides comprehensive technical details for configuring and utilizing external tool providers through the Model Context Protocol (MCP) with Synapse. This allows Synapse to leverage tools hosted by separate local or remote MCP-compliant servers.

> [!NOTE]
> For a quick start guide on adding MCP servers through the UI, see [MCP Setup](../guides/mcp-setup.md).

> [!NOTE]
> This guide covers Synapse as an MCP **client**. To expose Synapse as an MCP **server**, see [Connectivity → MCP Server](connectivity.md#mcp-server-connectivity).

## MCP Server Types

Synapse supports three main types of MCP servers:

1.  **Local Stdio Servers**: Local executables that Synapse communicates with via standard input/output (stdio).
...
## How Synapse Consumes MCP Tools

Synapse discovers and integrates MCP tools dynamically through the following process:

1.  **Configuration**: MCP servers are defined in the Synapse configuration, primarily through the Settings UI.
2.  **Saving Settings**: When saved via the UI, Synapse updates `usr/settings.json`, specifically the `"mcp_servers"` key.
3.  **Server Startup**: Synapse initializes configured MCP servers (stdio) or connects to them (remote). For `npx`/`uvx` based servers, the first run downloads packages.
4.  **Tool Discovery**: Upon initialization, Synapse connects to each enabled MCP server and queries for available tools, descriptions, and parameters.
...
6.  **Tool Invocation**: When the LLM requests an MCP tool, Synapse's `process_tools` method (`mcp_handler.py`) routes the request to the appropriate MCP server.
...
### Synapse in Docker, MCP Server on Host
...
- Final tool name in Synapse: `sequential_thinking.run_chain`
...
- Synapse maintains persistent connections to remote servers
...
Synapse can chain these tools automatically based on task requirements.
...
- Test with Synapse before production
...
- [Connectivity: MCP Server](connectivity.md#mcp-server-connectivity) - Exposing Synapse as MCP server
