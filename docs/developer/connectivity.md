# Synapse Connectivity Guide

This guide covers the different ways to connect to Synapse from external applications, including using the External API, connecting as an MCP client, and enabling agent-to-agent communication.

**Note:** You can find your specific URLs and API tokens in your Synapse instance under `Settings > External Services`.
...
Synapse provides external API endpoints for integration with other applications. These endpoints use API key authentication and support text messages and file attachments.

### `POST /api_message`

Send messages to Synapse and receive responses. Supports text messages, file attachments, and conversation continuity.
...
async function sendMessage() {
    try {
        const response = await fetch('YOUR_SYNAPSE_URL/api_message', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
...
async function sendMessageWithProject() {
    try {
        ...
        if (response.ok) {
            ...
            const followUp = await fetch('YOUR_SYNAPSE_URL/api_message', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-API-KEY': 'YOUR_API_KEY'
...
async function getLogsGET(contextId, length = 50) {
    try {
        ...
        const response = await fetch('YOUR_SYNAPSE_URL/api_log_get?' + params, {
            method: 'GET',
            headers: {
                'X-API-KEY': 'YOUR_API_KEY'
            }
...
async function getLogsPOST(contextId, length = 50) {
    try {
        const response = await fetch('YOUR_SYNAPSE_URL/api_log_get', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-API-KEY': 'YOUR_API_KEY'
...
async function terminateChat(contextId) {
    try {
        const response = await fetch('YOUR_SYNAPSE_URL/api_terminate_chat', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-API-KEY': 'YOUR_API_KEY'
...
async function resetChat(contextId) {
    try {
        const response = await fetch('YOUR_SYNAPSE_URL/api_reset_chat', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-API-KEY': 'YOUR_API_KEY'
...
async function getFiles(filePaths) {
    try {
        const response = await fetch('YOUR_SYNAPSE_URL/api_files_get', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-API-KEY': 'YOUR_API_KEY'
...
async function attachmentWorkflow() {
    ...
    const messageResponse = await fetch('YOUR_SYNAPSE_URL/api_message', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-API-KEY': 'YOUR_API_KEY'
...
Synapse includes an MCP Server that allows other MCP-compatible clients to connect to it. The server runs on the same URL and port as the Web UI.
...
> [!NOTE]
> This section describes **Synapse as an MCP server**. To configure Synapse as an MCP **client** (consuming external tools), see [MCP Setup](../guides/mcp-setup.md).

### Example MCP Server Configuration

Below is an example of a `mcp.json` configuration file that a client could use to connect to the Synapse MCP server. 
...
Synapse's A2A Server enables communication with other agents using the FastA2A protocol. Other agents can connect to your instance using the connection URL.

### A2A Connection URL

To connect another agent to your Synapse instance, use the following URL format. 

**Note:** You can find your specific A2A connection URL under `Settings > External Services > A2A Connection`.

```
YOUR_SYNAPSE_URL/a2a/t-YOUR_API_TOKEN
```

To connect with a specific project active:

```
YOUR_SYNAPSE_URL/a2a/t-YOUR_API_TOKEN/p-PROJECT_NAME
```
...
### Practical Use Cases
- Connect two Synapse instances for long-running, isolated workflows
