...
Synapse is built on a flexible and modular architecture designed for extensibility and customization. This section outlines the key components and the interactions between them.

## System Architecture

The user or Synapse is at the top of the hierarchy, delegating tasks to subordinate agents, which can further delegate to other agents. Each agent can utilize tools and access the shared assets (prompts, memory, knowledge, extensions and skills) to perform its tasks.

## Runtime Architecture
Synapse's runtime architecture is built around Docker containers:
...
2. **Runtime Container**:
   - Houses the complete Synapse framework
...
> [!NOTE]
> The legacy approach of running Synapse directly on the host system (using Python, Conda, etc.) 
...
> [!NOTE]
> In the Docker runtime, the framework lives under `/synapse` inside the container. Data persists as long as the container exists. For upgrades, prefer **Backup & Restore** instead of mapping the full `/synapse` directory.

## Core Components
Synapse's architecture revolves around the following key components:
...
Synapse employs a hierarchical agent structure, where a top-level agent (often the user) can delegate tasks to subordinate agents. This hierarchy allows for the efficient breakdown of complex tasks into smaller, more manageable sub-tasks.
...
#### Interaction Flow
A typical interaction flow within Synapse might look like this:

1. The user provides an instruction to Synapse
2. Synapse initializes VectorDB and access memory
3. Synapse analyzes the instruction and formulates a plan using `thoughts` argument, possibly involving the use of tools or the creation of sub-agents
4. If necessary, Synapse delegates sub-tasks to subordinate agents
...
7. Synapse provides the final response to the user

### 2. Tools
Tools are functionalities that agents can leverage. These can include anything from web search and code execution to interacting with APIs or controlling external software. Synapse provides a mechanism for defining and integrating both built-in and custom tools.

#### Built-in Tools
Synapse comes with a set of built-in tools designed to help agents perform tasks efficiently:

| Tool | Function |
| --- | --- |
| behavior_adjustment | Synapse use this tool to change its behavior according to a prior request from the user.
...
#### SearXNG Integration
Synapse has integrated SearXNG as its primary search tool, replacing the previous knowledge tools (Perplexity and DuckDuckGo). This integration enhances the agent's ability to retrieve information while ensuring user privacy and customization.
...
- Fallback Mechanism
In cases where SearXNG might not return satisfactory results, Synapse can be configured to fall back on other sources or methods, ensuring that users always have access to information.
...
#### Custom Tools
Users can create custom tools to extend Synapse's capabilities. Custom tools can be integrated into the framework by defining a tool specification, which includes the tool's prompt. Place these prompt overrides in your agent profile:
...
### 3. Memory System
The memory system is a critical component of Synapse, enabling the agent to learn and adapt from past interactions. It operates on a hybrid model where part of the memory is managed automatically by the framework while users can also manually input and extract information.
...
Synapse employs a sophisticated message history and summarization system to maintain context effectively while optimizing memory usage. This system dynamically manages the information flow, ensuring relevant details are readily available while efficiently handling the constraints of context windows.
...
- **Summarization Process:** Using natural language processing through the utility model, Synapse condenses the extracted information into concise summaries. By summarizing past interactions, Synapse can quickly recall important facts about the whole chat, leading to more appropriate responses.
...
By dynamically adjusting context windows and summarizing past interactions, Synapse enhances both efficiency and user experience. This innovation not only reflects the framework's commitment to being dynamic and user-centric, but also draws inspiration from human cognitive processes, making AI interactions more relatable and effective. Just as humans forget trivial details, Synapse intelligently condenses information to enhance communication.

> [!NOTE]
> To maximize the effectiveness of context summarization, users should provide clear and specific instructions during interactions. This helps Synapse understand which details are most important to retain.
...
3. Synapse merges these overrides with the default prompts automatically
...
> [!NOTE]  
> You can customize any of these files. Synapse will use files in `agents/<agent_profile>/prompts/` when present, and fall back to `prompts/` for everything else.
...
| Directory | Description |
|-----------|-------------|
| `/skills` | Default skills included with Synapse |
...
### 7. Extensions
Extensions are a powerful feature of Synapse, designed to keep the main codebase clean and organized while allowing for greater flexibility and modularity.
