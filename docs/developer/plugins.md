...
This page documents the current Synapse plugin system, including manifest format, discovery rules, scoped configuration, activation behavior, and how to share a plugin with the community.

## Overview

Plugins extend Synapse through convention-based folders. A plugin can provide:
...
Every plugin must contain `plugin.yaml`. This is the **runtime manifest** — it drives Synapse behavior. It is distinct from the index manifest used when publishing to the Plugin Index (see [Publishing to the Plugin Index](#publishing-to-the-plugin-index) below).
...
Plugins can also include an optional `hooks.py` at the plugin root. Synapse loads this module on demand and calls exported functions by name through `helpers.plugins.call_plugin_hook(...)`.

- `hooks.py` executes inside the **Synapse framework runtime and Python environment**.
...
- If `hooks.py` runs `sys.executable -m pip install ...`, it installs into the **same Python environment that is currently running Synapse**.
...
**Runtime manifest** (`plugin.yaml`, inside your plugin's own repo — drives Synapse behavior):
```yaml
...
Synapse now exposes the community **Plugin Hub** through the always-enabled **Plugin Installer** plugin. Users can browse Plugin Index entries directly from the Plugins UI without leaving the application.
