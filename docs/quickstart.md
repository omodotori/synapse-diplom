# Quick Start
This guide provides a quick introduction to using Synapse. We'll cover the recommended install path and running your first Skill.
...
## Use Synapse on your real local files

If you want Synapse to work on the actual files on your computer, this is the important part.

Synapse stays in Docker for safety. The A0 CLI installs and runs on your host machine. That host-side CLI is what lets Synapse work on the real files on your real computer.
...
> [!NOTE]
> Synapse supports any LLM provider, including local models via Ollama. For detailed provider configuration and local model setup, see the [Installation Guide](setup/installation.md#choosing-your-llms).

### Start your first chat

Once configured, you'll see the Synapse dashboard with access to:
...
Click **New Chat** to start creating with Synapse!

![Synapse Dashboard](res/quickstart/ui_newchat1.png)
...
## Example Interaction
Let's ask Synapse to use one of the built-in skills. Here's how:

1. Type "Activate your brainstorming skill" in the chat input field and press Enter or click the send button.
2. Synapse will process your request. You'll see its thoughts and tool calls in the UI.
...
## Next Steps
Now that you've run a simple task, you can experiment with more complex requests. Try asking Synapse to:
