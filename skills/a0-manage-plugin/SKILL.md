...
> "Before installing, I strongly recommend running a security scan on this plugin. Third-party plugins execute code inside your Synapse environment. Should I scan it first? The scan typically takes 2-4 minutes."
...
> **Always use the HTTP API or UI.** Never import Synapse framework modules directly from `code_execution_tool` - the agent runs in a separate Python runtime (`/opt/venv`) that does not have the framework's dependencies. All programmatic installs must go through HTTP.
...
The Synapse API uses CSRF protection. The `Origin` header is **always required** - without it the CSRF endpoint returns `ok: false` even when login is disabled.

**Step 1: Set the base URL.** Synapse listens on port 80 inside Docker (the standard deployment):
...
Or simply restart Synapse - on startup it re-scans `usr/plugins/` fresh.
