...
trigger_patterns:
  ...
  - "connect a0 to synapse"
  - "connect synapse to local files"
  - "let synapse work on my local files"
...
Use this skill to guide the user through installing `a0` on their host machine and connecting it to Synapse.

## Core Boundary

- Synapse stays in Docker or its sandboxed runtime.
- `a0` installs and runs on the user's host machine.
- The whole point is to let Synapse work on the real files on the user's computer.
...
- If Synapse is running locally, `a0` may discover it automatically.
- If not, the user can enter the Synapse web URL manually in the custom URL field.
...
- `SYNAPSE_HOST` can prefill the target host without bypassing the picker.

Example:

```bash
export SYNAPSE_HOST=http://localhost:50001
a0
```
...
```bash
export SYNAPSE_HOST=https://example.trycloudflare.com
a0
```
...
- It connects to the user's Synapse instance or reaches the login step.
- The user can open a chat from the terminal.
- Synapse can now act on real files on the host through the connector flow while Synapse itself still runs in Docker.
...
User: "I'm inside the Synapse container. How do I install A0?"
...
- Then tell them to run `a0` and connect to Synapse.
