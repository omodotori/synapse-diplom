...
- `hooks.py` runs inside the **Synapse framework runtime**, not the separate agent execution environment.
...
- If `hooks.py` runs `sys.executable -m pip install ...`, it installs into the same Python environment that is running Synapse.
