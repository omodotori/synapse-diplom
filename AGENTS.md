...
Synapse is a dynamic, organic agentic framework designed to grow and learn. It uses the operating system as a tool, featuring a multi-agent cooperation model where every agent can create subordinates to break down tasks.
...
When running in Docker, Synapse uses two distinct Python runtimes to isolate the framework from the code being executed:

### 1. Framework Runtime (/opt/venv-a0)
- Version: Python 3.12.4
- Purpose: Runs the Synapse backend, API, and core logic.
...
- Hook runtime: hooks.py executes inside the Synapse framework Python environment, so sys.executable -m pip installs dependencies into that same framework runtime.
...
*Maintained by: Synapse Core Team*
